...
client.once('ready', async () => {
    console.log(`✅ ${client.user.tag} جاهز ونظام نقل التذاكر للكاتيجوري مفعل!`);
...
client.on('interactionCreate', async (i) => {
  ...
  if (i.customId === 'open_ticket') {
    const channel = await i.guild.channels.create({
      ...
      parent: TICKET_CATEGORY_ID, // 👈 ربط الروم تلقائياً ليكون داخل القسم الخاص بك وتحت آخر شيء
  ...
  if (i.customId === 'claim_ticket') {
    if (!STAFF_ROLES.some(r => i.member.roles.cache.has(r))) return i.reply({ content: 'هذا الزر مخصص للإدارة فقط!', ephemeral: true });
  ...
  if (i.customId === 'confirm_close_request') {
    if (!STAFF_ROLES.some(r => i.member.roles.cache.has(r))) return i.reply({ content: 'مخصص للإدارة فقط!', ephemeral: true });
    const row = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('final_delete').setLabel('تأكيد الإغلاق').setStyle(ButtonStyle.Danger));
...
// --- معالجة أوامر البريفكس العادية (+come, +add, +rename, +close) ---
