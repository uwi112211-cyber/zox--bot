...
// --- وظيفة مساعدة محسنة ومحمية لتخطي الرتب الأعلى من البوت تلقائياً ---
    ...
    try {
      if (overwrite.id === sourceChannel.guild.roles.everyone.id) {
        ...
        if (sourceRole) {
          ...
          if (targetRole) {
            // التحقق وتخطي الرتب الحساسة أو الأعلى لعدم تجميد الكود
            newOverwrites.push({
              id: targetRole.id,
              allow: overwrite.allow.toArray(),
              deny: overwrite.deny.toArray()
      ...
      // تجاهل الرتبة المسببة للمشكلة والاستمرار في نسخ الباقي
      console.log(`⚠️ تم تخطي صلاحية رتبة معينة في روم ${sourceChannel.name} بسبب قيود صلاحيات ديسكورد.`);
    }
  });
...
client.on('interactionCreate', async (i) => {
  ...
  if (i.isChatInputCommand()) {
      ...
      // 🔄 أمر نسخ ونقل السيرفر المطور والمحمي من التوقف بسبب ترتيب الرتب
      if (i.commandName === 'clone-server') {
          ...
          await i.deferReply();
          await i.editReply({ content: '⏳ | جاري النقل المنظم للأقسام والرومات وصلاحياتها... يرجى الانتظار.' });
          ...
          try {
              ...
              // 1. إنشاء الرومات العادية المبعثرة بالأعلى
              for (const [_, channel] of orphanChannels) {
                  ...
                  }).catch(() => {}); // حماية عدم توقف الكود
              }

              // 2. إنشاء الأقسام وداخلها روماتها بالترتيب
              for (const [_, category] of categories) {
                  ...
                  // إذا فشل إنشاء قسم معين لسبب ما، يكمل البوت الباقي
              ...
              const successEmbed = new EmbedBuilder()
                  .setTitle('✅ تم نسخ السيرفر بالكامل وبمرونة عالية!')
                  .setDescription(`قام المسؤول ${i.user} بنسخ السيرفر بنجاح، تم تخطي الرتب التي تتعارض مع صلاحيات البوت لإكمال النقل بدون مشاكل.`)
                  .addFields(
              ...
              console.error(err);
              return i.editReply({ content: '❌ | حدث خطأ غير متوقع أثناء النقل. تأكد من إعطاء البوت صلاحية إدارة القنوات على الأقل.' });
          }
      }
  }
