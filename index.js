...
// كولكشن داخلي لحفظ آيدي الإداري المستلم لكل تذكرة
const claimedTickets = new Map();

// --- تسجيل أوامر السلاش عند جاهزية البوت ---
client.once('ready', async () => {
    console.log(`✅ ${client.user.tag} جاهز! تم منع منشئ التذكرة من استلامها.`);
...
client.on('interactionCreate', async (i) => {
  ...
  // 🔒 زر استلام التذكرة (محمي للإدارة + يمنع منشئ التذكرة من استلامها)
  if (i.customId === 'claim_ticket') {
    if (!STAFF_ROLES.some(r => i.member.roles.cache.has(r))) {
      return i.reply({ content: '❌ هذا الزر مخصص لطاقم الإدارة والدعم الفني فقط!', ephemeral: true });
    }
    
    // فحص صلاحيات الروم لمعرفة من قام بفتح التذكرة (البوت يبحث عن الصلاحية الخاصة بالمستخدم العادي المسموح له بالقراءة والكتابة)
    const creatorOverwrite = i.channel.permissionOverwrites.cache.find(p => p.type === 1 && p.id !== client.user.id && !STAFF_ROLES.includes(p.id));
    
    // إذا تطابق آيدي الشخص اللي يضغط الزر مع آيدي منشئ التيكت، يتم منعه فوراً
    if (creatorOverwrite && creatorOverwrite.id === i.user.id) {
      return i.reply({ content: '❌ لا يمكنك استلام تذكرة قمت بفتحها بنفسك! اتركها لإداري آخر ليقوم بمساندتك.', ephemeral: true });
    }
    
    // حفظ آيدي الإداري الذي استلم هذه التذكرة تحديداً
    claimedTickets.set(i.channel.id, i.user.id);
  ...
  // 🔒 زر إلغاء استلام التذكرة (محمي ومقفل: لا يسمح إلا للشخص المستلم نفسه!)
  if (i.customId === 'unclaim_ticket') {
    if (!STAFF_ROLES.some(r => i.member.roles.cache.has(r))) {
      return i.reply({ content: '❌ هذا الزر مخصص لطاقم الإدارة والدعم الفني فقط!', ephemeral: true });
    }
    
    const staffWhoClaimed = claimedTickets.get(i.channel.id);
    if (staffWhoClaimed && staffWhoClaimed !== i.user.id) {
      return i.reply({ content: '❌ أنت لست المشرف الذي قام باستلام هذه التذكرة لإلغائها!', ephemeral: true });
    }
    
    // مسح آيدي المستلم من الذاكرة
    claimedTickets.delete(i.channel.id);
  ...
  // 🔒 زر طلب إغلاق التذكرة (ممنوع تماماً على العضو اللي فتح التيكت!)
  if (i.customId === 'confirm_close_request') {
    if (!STAFF_ROLES.some(r => i.member.roles.cache.has(r))) {
      return i.reply({ content: '❌ هذا الزر مخصص لطاقم الإدارة فقط! لا يمكنك إغلاق التذكرة بنفسك.', ephemeral: true });
    }
    const row = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('final_delete').setLabel('تأكيد الإغلاق').setStyle(ButtonStyle.Danger));
    return i.reply({ content: '⚠️ هل أنت متأكد من إغلاق وحذف التذكرة؟', components: [row] });
  }

  // 🔒 زر الحذف النهائي الفعلي (محمي للإدارة فقط وممنوع للعضو)
